...
public final class DropDown: UIView {
    ...
    fileprivate let tfSearch = UITextField()
    ...
    */
    ...
    @objc fileprivate dynamic var tableViewBackgroundColor = DPDConstant.UI.BackgroundColor {
        willSet {
            ...
            tfSearch.backgroundColor = newValue
    ...
    */
    ...
    @objc public dynamic var placeholderColor: UIColor = DPDConstant.UI.TextColor {
        didSet {
            tfSearch.attributedPlaceholder = NSAttributedString(string: placeholder, attributes: [NSAttributedString.Key.foregroundColor : placeholderColor])
    ...
    */
    ...
    private var searchDataSource = [String]()
    ...
    public var localizationKeysDataSource = [String]() {
        didSet {
            searchDataSource = localizationKeysDataSource.map { NSLocalizedString($0, comment: "") }
        }
    }
    ...
    */
    public init(searchable: Bool = false) {
    ...
    */
    public convenience init(anchorView: AnchorView, selectionAction: SelectionClosure? = nil, dataSource: [String] = [], topOffset: CGPoint? = nil, bottomOffset: CGPoint? = nil, cellConfiguration: ConfigurationClosure? = nil, cancelAction: Closure? = nil, searchable: Bool = false) {
        self.init(searchable: searchable)
...
private extension DropDown {
    ...
    func setupUI() {
        ...
        tfSearch.leftView = UIView(frame: CGRect(x: 0, y: 0, width: 5, height: 20))
        tfSearch.leftViewMode = .always
        tfSearch.textColor = textColor
        tfSearch.font = textFont
        tfSearch.backgroundColor = tableViewBackgroundColor
        tableViewContainer.addSubview(self.tfSearch)
        tfSearch.addTarget(self, action: #selector(onTextSearchChanged), for: .editingChanged)
        self.tfSearch.translatesAutoresizingMaskIntoConstraints = false
        
        lineView.translatesAutoresizingMaskIntoConstraints = false
        lineView.backgroundColor = separatorColor
        tableViewContainer.addSubview(lineView)
    }
    
    @objc private func onTextSearchChanged(sender: UITextField)  {
        ...
        self.searchDataSource = self.dataSource.filter {
            return textSearch.isEmpty || $0.localizedCaseInsensitiveContains(textSearch)
        }
        deselectRows(at: selectedRowIndices)
        reloadAllComponents()
    }
...
//        tableViewContainer.addUniversalConstraints(format: "|[tableView]|", views: ["tableView": tableView])
        if isSearchable {
            NSLayoutConstraint.activate([
                tfSearch.topAnchor.constraint(equalTo: tableViewContainer.topAnchor),
                tfSearch.leadingAnchor.constraint(equalTo: tableViewContainer.leadingAnchor),
                tfSearch.trailingAnchor.constraint(equalTo: tableViewContainer.trailingAnchor),
                tfSearch.heightAnchor.constraint(equalToConstant: cellHeight)
            ])
            
            NSLayoutConstraint.activate([
                lineView.topAnchor.constraint(equalTo: tfSearch.bottomAnchor),
                lineView.leadingAnchor.constraint(equalTo: tableViewContainer.leadingAnchor),
                lineView.trailingAnchor.constraint(equalTo: tableViewContainer.trailingAnchor),
                lineView.heightAnchor.constraint(equalToConstant: 1)
            ])
            width = UIScreen.main.bounds.width / 3
        }
        NSLayoutConstraint.activate([
            tableView.topAnchor.constraint(equalTo: isSearchable ? lineView.bottomAnchor : tableViewContainer.topAnchor),
    ...
    fileprivate func computeLayoutBottomDisplay(window: UIWindow) -> ComputeLayoutTuple {
        ...
        if isSearchable {
            width = max(max(width, fittingWidth()), anchorView?.plainView.bounds.width ?? 0)
        }
    ...
    fileprivate func computeLayoutForTopDisplay(window: UIWindow) -> ComputeLayoutTuple {
        ...
        var width = self.width ?? (anchorView?.plainView.bounds.width ?? fittingWidth()) - topOffset.x
        if isSearchable {
            width = max(max(width, fittingWidth()), anchorView?.plainView.bounds.width ?? 0)
        }
        return (x, y, width, offscreenHeight)
    ...
    fileprivate func fittingWidth() -> CGFloat {
        ...
        for index in 0..<searchDataSource.count {
...
extension DropDown {
    ...
    */
    ...
    public var selectedItem: String? {
        ...
        return searchDataSource[row]
    }

    /// Returns the height needed to display all cells.
    fileprivate var tableHeight: CGFloat {
        return (tableView.rowHeight * CGFloat(searchDataSource.count) ) + (isSearchable ? cellHeight : 0 )
    }
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
    ...
    fileprivate func configureCell(_ cell: DropDownCell, at index: Int) {
        ...
        if let cellConfiguration = cellConfiguration {
            cell.optionLabel.text = cellConfiguration(index, searchDataSource[index])
        } else {
            cell.optionLabel.text = searchDataSource[index]
        }
        if  isSearchable {
            guard let _index = dataSource.firstIndex(where: { $0 == searchDataSource[index] }) else { return }
            customCellConfiguration?(_index, searchDataSource[index], cell)
        } else {
            customCellConfiguration?(index, searchDataSource[index], cell)
        }
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        var selectedRowIndex = (indexPath as NSIndexPath).row
        ...
        if let multiSelectionCallback = multiSelectionAction {
            ...
            if selectedRowIndices.first(where: { $0 == selectedRowIndex}) != nil {
                ...
                let selectedRows = selectedRowIndicesArray.map { searchDataSource[$0] }
                ...
                let selectedRowIndicesArray = Array(selectedRowIndices)
                let selectedRows = selectedRowIndicesArray.map { searchDataSource[$0] }
                selectionAction?(selectedRowIndex, searchDataSource[selectedRowIndex])
                multiSelectionCallback(selectedRowIndicesArray, selectedRows)
                tableView.reloadData()
            }
            return
        }
        if isSearchable, let _index = dataSource.firstIndex(where: { $0 == searchDataSource[indexPath.row] }) {
            selectedRowIndex = _index
        }
